"""
Skill extraction (SkillNER) for preprocessed CaribbeanJobs jobs.

Input : data/processed/processed_jobs.jsonl
Output: out/caribbeanjobs_skills.jsonl

Expected input fields per record:
- region (should be "Caribbean")
- job_category
- job_title
- clean_text
...
OUTPUT_FILE = Path("out/caribbeanjobs_skills.jsonl")
...
def extract_unique_skills(annotation: Dict[str, Any]) -> List[str]:
    ...
    Returns simple list of skill strings.
    """
...
def main(
    ...
    # Load spaCy model (lightweight load)
    try:
        nlp = spacy.load(
            spacy_model,
    ...
    # 🔴 FILTER: Only process Caribbean records
            if region != "Caribbean":
                continue
            ...
            count += 1
            if count % 100 == 0:
                print(f"[INFO] Extracted skills for {count} Caribbean records...")

    write_jsonl(output_file, gen_out())
    print(f"[DONE] Wrote Caribbean skills to: {output_file.resolve()}")
