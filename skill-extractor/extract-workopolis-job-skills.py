...
def main(
    ...
    def gen_out() -> Iterator[Dict[str, Any]]:
        ...
        for rec in read_jsonl(input_file):
            ...
            # 🔵 FILTER: Only process Canada (Workopolis)
            if region != "Canada":
                continue
            ...
            count += 1
            if count % 100 == 0:
                print(f"[INFO] Extracted skills for {count} Canadian records...")

    write_jsonl(output_file, gen_out())
    print(f"[DONE] Wrote Workopolis (Canada) skills to: {output_file.resolve()}")
